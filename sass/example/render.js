...
Ext.theme = Ext.apply(Ext.theme || {}, {
    ...
    addShortcuts (shortcuts) {
        let all = Ext.theme._shortcuts;

        for (let key in shortcuts) {

            let add = shortcuts[key];
            let xtype = Ext.theme.addWidget(key);
            let existing = all[xtype];
     ...
     * @private
     */
    applyShortcut (manifestEntry, shortcut) {
        let ui = manifestEntry.ui;
        let config = Ext.theme.copyProps({}, manifestEntry.config);
        let entry = Ext.theme.copyProps({}, manifestEntry);
     ...
     * @private
     */
    renderWidget (manifestEntry, shortcut) {
        let entry = Ext.theme.applyShortcut(manifestEntry, shortcut);
        let config = entry.config;
        let widget = Ext.create(entry.xtype, config);
        let ct = Ext.fly(document.body).createChild({ cls: 'widget-container' });
    ...
    render () {
        ...
        let shortcuts = Ext.theme._shortcuts;

        for (let k = 0, n = manifest ? manifest.length : 0; k < n; ++k) {
            let manifestEntry = manifest[k];
            let xtype = Ext.theme.addWidget(manifestEntry.xtype);
            let widgetShortcuts = xtype ? shortcuts[xtype] : null;
     ...
     */
    run () {
        let extjsVer = Ext.versions.extjs;
        let globalData = {};

        if (Ext.layout.Context) {
            Ext.override(Ext.layout.Context, {
                run () {
                    let ok = this.callParent(),
